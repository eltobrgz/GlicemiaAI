...
import { formatTime } from '@/lib/utils'; // Re-using formatTime from utils
import { Button } from '@/components/ui/button';
import { Trash2, PlusCircle, Loader2, Pill } from 'lucide-react';
              ...
              <div key={log.id} className="p-3 border rounded-md bg-card hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">{formatTime(format(parseISO(log.timestamp), 'HH:mm'))}</p> {/* formatTime expects HH:mm */}
                  </div>
